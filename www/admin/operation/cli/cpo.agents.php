<?php
    ...
    function getUser() {
        $users = $GLOBALS['db']->selectAll("SELECT * FROM pbx_campaign_agent C INNER JOIN pbx_locker L ON C.pbx_locker_id = L.pbx_locker_id WHERE C.pbx_campaign_id = ". $_SESSION[session_key] . " AND " . query_filter());
        if($users) {
            return $users;
        } else {
            return false;
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(0), 116);
    $table->displayTable();
?>
