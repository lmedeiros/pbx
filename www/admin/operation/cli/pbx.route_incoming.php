<?php
    ...
    function getUser() {
        $users = $GLOBALS['db']->selectAll("SELECT * FROM pbx_route_incoming WHERE pbx_campaign_id = " . $_SESSION[session_key] . " AND " . query_filter());
        if($users) {
            return $users;
        } else {
            return false;
    ...
    $actions = array(129, 130);
    $actions_confirm = Array(130=>'Tem certeza que deseja apagar essa rota?');
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(127),116);
    $table->displayTable();
?>
