<?php
    ...
    function getUser() {
        $users = $GLOBALS['db']->selectAll("SELECT * FROM pbx_peer_sip P INNER JOIN pbx_campaign_peer C ON C.pbx_peer_id = P.accountcode AND C.pbx_campaign_id = " . $_SESSION[session_key]  . " AND " . query_filter());
        if($users) {
            return $users;
        } else {
            return false;
