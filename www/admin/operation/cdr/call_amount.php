<?php
    ...
    function getCalls() {
        ...
        $calls = $GLOBALS['db']->selectAll("SELECT count(calldate) as amount, clid FROM pbx_cdr WHERE calldate BETWEEN '{$datestart}' AND '{$dateend}' AND clid LIKE '{$clid}' AND dst LIKE '{$dst}' AND dcontext='global' AND dst NOT LIKE '%BUSY%' AND dst NOT LIKE '%NOANSWER%' AND dst NOT LIKE '%CONGESTION%' GROUP BY clid");
        if($calls) {
            return $calls;
        } else {
            return false;
