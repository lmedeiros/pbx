<?php
    ...
    function getAccounts() {
        $accounts = $GLOBALS['db']->selectAll("SELECT * FROM pbx_account WHERE prepaid='1' AND " . query_filter());
        if($accounts) {
            return $accounts;
        } else {
            return false;
    ...
    function get_credits($value) {
        ...
        if($account) {
            ...
            if($accounts) {
                return $accounts['symbol'] . " " . round($account['credit']*$accounts['convertion_rate'], 2);
            } else {
                return false;
            }
        } else {
    ...
    $headers = array("Nome", "#", "Créditos" ,"Ações");
    ...
    $actions = array(34,44 ,33, 54, 32, 35,85,56);
    $actions_confirm = Array(33=>'Tem certeza que deseja excluir esse Ramal?');
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(30),53);
    $table->displayTable();
?>
