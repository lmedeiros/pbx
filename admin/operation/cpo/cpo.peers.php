<?php
    ...
    function getUser() {
        $users = $GLOBALS['db']->selectAll("SELECT * FROM pbx_peer_sip P INNER JOIN pbx_campaign_peer C ON C.pbx_peer_id = P.accountcode AND C.pbx_campaign_id = " . $_GET['id']  . " AND " . query_filter());
        if($users) {
            return $users;
        } else {
            return false;
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array($_GET['id']=>101),95);
    $table->displayTable();
?>
