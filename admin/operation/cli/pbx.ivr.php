<?php
    ...
    function getUser() {
        $users = $GLOBALS['db']->selectAll("SELECT * FROM pbx_ivr WHERE pbx_campaign_id = " . $_SESSION[session_key]  . " AND " . query_filter());
        if($users) {
            return $users;
        } else {
            return false;
    ...
    $actions = array(122, 123, 124);
    $actions_confirm = Array(123=>'Certeza que deseja apagar essa URA?');
    ...
    #show tables
    new CreateHome(@$_GET['operation'], array(121),116);
    $table->displayTable();
?>
