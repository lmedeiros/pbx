...
function load_values() {
    ...
    if (isset($_GET['id']) && $_GET['id'] != "") {
        ...
        foreach (get_routes() as $route => $desc) {
            $query = "SELECT count(is_allowed) as is_allowed FROM pbx_route_permission INNER JOIN pbx_campaign_peer ON pbx_peer_id = pbx_peer_sip_id WHERE pbx_campaign_id = " . $_SESSION[session_key] . " AND pbx_peer_sip_id = '{$_GET['id']}' AND code='{$route}' AND is_allowed='1' ";
