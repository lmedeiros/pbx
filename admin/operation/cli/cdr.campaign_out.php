<?php
    ...
    $headers = array("Data", "Ramal", "Agente ", "Número", "Localidade" , "Status","Duração", "Valor","");
    $columns = array("calldate", "accountcode", "locker", "dst", "location", "disposition","billsec", "cost", "ACTIONS");

    $table = new DisplayTable(getCalls(),'calldate', 'DESC', 50);
    $table->setHeaders($headers);
    $table->setColumns($columns);
    $table->setEmptyMessage("There is no accounts.");
    new CreateHome(@$_GET['operation'], array(0),137);
    $table->displayTable();
?>
